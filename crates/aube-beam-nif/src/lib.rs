...
fn run_cli<'a>(env: Env<'a>, args: Vec<String>) -> NifResult<Term<'a>> {
    static CLI_USED: AtomicBool = AtomicBool::new(false);
    if CLI_USED
        ...
        .is_err()
    {
        return Ok((
            atoms::error(),
            RunFailure {
                message: "the embedded CLI can only run once per BEAM VM".to_string(),
                code: Some(aube_codes::errors::ERR_AUBE_EMBED_ALREADY_INITIALIZED.to_string()),
                exit_code: 1,
            },
        )
            .encode(env));
    ...
    let result = std::thread::Builder::new()
        .name("aube-beam-cli".to_string())
        .stack_size(16 * 1024 * 1024)
        .spawn(move || {
            let argv = std::iter::once(OsString::from("aube"))
                .chain(args.into_iter().map(OsString::from))
                .collect();
            aube::cli_main_from(&embed::AUBE, argv)
        });
    let result = match result {
        Ok(handle) => handle.join().map_err(|_| RunFailure {
            message: "the embedded aube CLI thread panicked".to_string(),
            code: Some(aube_codes::errors::ERR_AUBE_FFI_PANIC.to_string()),
            exit_code: 1,
        }),
        Err(error) => Err(RunFailure {
            message: format!("failed to start the embedded aube CLI thread: {error}"),
    ...
    Ok(match result {
        Ok(exit_code) => (atoms::ok(), RunResult { exit_code }).encode(env),
