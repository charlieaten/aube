...
/// Run the CLI with process arguments and return the process exit code.
pub fn run_from_env() -> i32 {
    // Two-phase wrapper: `run_cli` runs the real CLI and returns
    // `Result<Option<i32>, miette::Report>`. On Err we render via miette's
    ...
    let result = run_cli(std::env::args_os().collect());
    flush_cli_diagnostics();
    match result {
...
pub(crate) fn flush_cli_diagnostics() {
    aube_util::diag::flush();
...
pub(crate) fn report_exit_code(report: &miette::Report) -> i32 {
...
pub fn run_cli(argv: Vec<OsString>) -> miette::Result<Option<i32>> {
    run_cli_with_host(argv, CliHost::Process)
}

pub(crate) fn run_cli_embedded(argv: Vec<OsString>) -> miette::Result<Option<i32>> {
    run_cli_with_host(argv, CliHost::Embedded)
}

fn run_cli_with_host(mut argv: Vec<OsString>, host: CliHost) -> miette::Result<Option<i32>> {
    ...
    let cli = match Cli::try_parse_from(lift_per_subcommand_flags(rewrite_multicall_argv(argv))) {
        Ok(cli) => cli,
        Err(err) => {
    ...
    if matches!(host, CliHost::Process) && matches!(color_mode, ColorMode::Never) {
    ...
    } else if matches!(host, CliHost::Process) && matches!(color_mode, ColorMode::Always) {
        // SAFETY: single-threaded `main` — no other threads exist yet.
        unsafe {
    ...
    } else if matches!(host, CliHost::Process) && ci_renders_ansi() && !env_disables_color() {
    ...
    if matches!(host, CliHost::Process) && !is_silent {
    ...
    let exit_code = runtime.block_on(async_main(cli, host))?;
    drop(runtime);
    Ok(exit_code)
}

async fn async_main(cli: Cli, host: CliHost) -> miette::Result<Option<i32>> {
    ...
    let _silent_guard = (matches!(host, CliHost::Process)
        && matches!(effective_level, LogLevel::Silent))
