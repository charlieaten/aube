defmodule Aube do
  ...
  def install(_opts) do
    {:error,
     %{
       message: "install options must be a keyword list",
       code: "ERR_AUBE_FFI_INVALID_ARGUMENT"
     }}
  end

  @doc """
  Runs one complete Aube CLI invocation inside the current BEAM VM.

  The CLI initializes process-global state and can therefore only be invoked
  once per VM. Use `install/1` for repeatable, invocation-scoped installs.
  """
  ...
  def run(_args), do: invalid_run_args()
  ...
  defp validate_mutual_exclusion(opts) do
    ...
    cond do
      frozen_count > 1 ->
        {:error,
         %{
           message: "frozen lockfile options are mutually exclusive",
           code: "ERR_AUBE_FFI_INVALID_ARGUMENT"
         }}
      ...
      truthy?(opts[:prod]) and truthy?(opts[:dev]) ->
        {:error,
         %{
           message: "prod and dev install modes are mutually exclusive",
           code: "ERR_AUBE_FFI_INVALID_ARGUMENT"
         }}
      ...
      truthy?(opts[:offline]) and truthy?(opts[:prefer_offline]) ->
        {:error,
         %{
           message: "offline and prefer_offline are mutually exclusive",
           code: "ERR_AUBE_FFI_INVALID_ARGUMENT"
         }}
          ...
          true ->
            {:error,
             %{
               ...
               code: "ERR_AUBE_EMBED_INVALID_PROJECT"
             }}
        end
    end
  end
