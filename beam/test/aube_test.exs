defmodule AubeTest do
  ...
  test "invalid cwd returns a clean error" do
    ...
    assert {:error, %{code: "ERR_AUBE_EMBED_INVALID_PROJECT", message: message}} =
             Aube.install(cwd: cwd)

    assert message =~ "cwd is not a directory"
  end

  test "unknown install options are rejected" do
    assert {:error, %{code: "ERR_AUBE_FFI_INVALID_ARGUMENT", message: message}} =
  ...
  test "run validates argv shape before entering the native CLI" do
    assert {:error, %{code: "ERR_AUBE_FFI_INVALID_ARGUMENT", exit_code: 1, message: message}} =
             Aube.run(["install", :not_a_string])
