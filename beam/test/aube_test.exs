defmodule AubeTest do
  ...
  test "run validates argv shape" do
    assert {:error, %{code: "invalid_args", exit_code: 1, message: message}} =
             Aube.run(["install", :not_a_string])

    assert message =~ "list of strings"
  end

  test "run exposes the full cli parser" do
    tmp = tmp_project!("aube_elixir_run")

    assert {:ok, %{exit_code: 0}} =
             Aube.run(["--silent", "--dir", tmp, "i", "--lockfile-only", "--ignore-scripts"])
